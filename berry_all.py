...
url = "http://192.168.1.97:6400/api/events/test"

def send_tag_data(tag_id):  

    r = requests.post(url, json={'lineNumber': 1, 'tagid': tag_id, 'dateTime': str(datetime.datetime.now())})
            ...
            if bytesToRead > 0:
                ...
                if len(actual_data) > 14:
                    ...
                    try:
                        ...
                        all_ids.append(_id_str)
                        all_ids = set(all_ids)
                        all_ids = list(all_ids)
                        print(f"TAGS FOUND: {len(set(all_ids))}\n{all_ids}")
                        if len(list(set(all_ids))) == 4:
                            r = requests.post(url, json={'lineNumber': 1, 'all_tags': str(all_ids), 'dateTime': str(datetime.datetime.now())})
                            print((all_ids, r.status_code, r.reason))
